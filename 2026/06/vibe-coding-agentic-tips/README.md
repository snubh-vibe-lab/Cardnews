...
- PNG 렌더: `Design/project/card_news/render_cards.py` (`python render_cards.py`)
  - 16장을 2160×2160로 렌더하고 contact_sheet까지 생성
  - 주의: 헤드리스 Chrome 창은 장식 때문에 실제 뷰포트가 요청보다 ~98px 작다.
    `--window-size=1080,1080`로 찍으면 뷰포트가 982px가 되어 푸터 하단이 잘린다.
    그래서 `--window-size=1180,1180`로 렌더한 뒤 좌상단 2160×2160만 크롭한다.
